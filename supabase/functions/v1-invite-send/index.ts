 ...
 *
 * Edge cases handled:
 * - EC-2.1: Invite existing member - handled gracefully in invite-accept
...
serve(async (req: Request): Promise<Response> => {
  ...
  try {
    ...
    // NOTE: EC-2.1 (invite existing member) is handled gracefully in v1-invite-accept
    // We cannot check auth.users from anon key, so we allow the invite to be created
    // and handle the "already member" case when they accept
