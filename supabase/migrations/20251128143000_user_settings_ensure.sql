-- Ensure user_settings exists with RLS and audit triggers
begin;
...
-- Add FK to auth.users if missing (Supabase pattern)
do $$
begin
  if not exists (
    select 1
...
-- updated_at trigger helper (shared elsewhere; safe to replace)
...
-- Audit trigger (function is shared; safe to create/replace)
