// packages/engine/src/policy-underwrite.ts
...
export type InvestorDiscounts = {
  ...
  p20_zip?: number; // e.g., 0.28
  /** ZIP typical investor discount (modal band) */
  typical_zip?: number; // e.g., 0.22
};
...
/**
 * Default underwriting policy.
 * Note: `investor_discounts` is intentionally undefined here.
 * Inject real ZIP-driven values at runtime or override in callers.
 */
export const UNDERWRITE_POLICY: UnderwritePolicy = {
  default_cash_close_add_days: 35,
  carry_month_cap: 5,
  mao_aiv_cap_pct: 0.97,
  investor_discounts: undefined, // [INFO NEEDED]: wire real ZIP dataset
};
