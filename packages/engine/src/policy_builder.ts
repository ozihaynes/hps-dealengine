...
const clonePolicy = (p: UnderwritingPolicy | null | undefined): UnderwritingPolicy =>
  JSON.parse(JSON.stringify(p ?? {}));

const pctToDecimal = (n: number | null | undefined): number | null => {
  if (n == null) return null;
  if (n > 1 || n > 0.5) return n / 100;
  if (n >= 0) return n;
  return null;
};

const sanitizeBands = (bands: any[] | undefined | null): UnderwritingPolicy["min_spread_by_arv_band"] | undefined => {
  ...
  const normalized = bands
    .map((b) => {
      ...
      const min_spread_dollars =
        typeof b?.minSpread === "number"
          ? b.minSpread
...
export function buildUnderwritingPolicyFromOptions(
  ...
  sandboxOptions: AnalyzeSandboxOptions | null | undefined,
): UnderwritingPolicy {
  const policy = clonePolicy(basePolicy);
  if (!sandboxOptions) return policy;
  ...
  if (sandboxOptions.valuation) {
    ...
    if (v.aivSafetyCapPercentage !== undefined) val.aiv_safety_cap_pct = v.aivSafetyCapPercentage;
    if (v.aivHardMax !== undefined) val.aiv_hard_max = v.aivHardMax;
    if (v.aivHardMin !== undefined) val.aiv_hard_min = v.aivHardMin;
    if (v.aivSoftMaxVsArvMultiplier !== undefined) val.aiv_soft_max_vs_arv_multiplier = v.aivSoftMaxVsArvMultiplier;
    if (v.arvHardMax !== undefined) val.arv_hard_max = v.arvHardMax;
    if (v.arvHardMin !== undefined) val.arv_hard_min = v.arvHardMin;
    if (v.arvSoftMaxVsAivMultiplier !== undefined) val.arv_soft_max_vs_aiv_multiplier = v.arvSoftMaxVsAivMultiplier;
  ...
  if (sandboxOptions.floorsSpreads?.minSpreadByArvBand) {
    ...
    if (bands) policy.min_spread_by_arv_band = bands;
  ...
  if (sandboxOptions.carryTimeline) {
    ...
    if (sandboxOptions.carryTimeline.daysToMoneySelectionMethod) {
      policy.dtm.selection_method = sandboxOptions.carryTimeline.daysToMoneySelectionMethod as any;
    }
  ...
  if (sandboxOptions.holdCosts) {
    ...
    if (hc.holdCostsFlipFastZip !== undefined) flip.fast = { monthly_pct_of_arv: pctToDecimal(hc.holdCostsFlipFastZip) };
    if (hc.holdCostsFlipNeutralZip !== undefined)
      flip.neutral = { monthly_pct_of_arv: pctToDecimal(hc.holdCostsFlipNeutralZip) };
    if (hc.holdCostsFlipSlowZip !== undefined) flip.slow = { monthly_pct_of_arv: pctToDecimal(hc.holdCostsFlipSlowZip) };
  ...
  if (sandboxOptions.profit_and_fees) {
    ...
    if (p.assignmentFeeTarget !== undefined) ensureAssignment().target_dollars = p.assignmentFeeTarget ?? null;
    ...
    if (p.buyerSegmentationWholetailMaxRepairsAsArvPercentage !== undefined) {
      ensureWholetailMargin().max_repairs_pct_of_arv = pctToDecimal(
        p.buyerSegmentationWholetailMaxRepairsAsArvPercentage,
      );
    }
  ...
  if (sandboxOptions.disposition_and_double_close) {
    ...
    if (d.dispositionTrackEnablement) ensureDisposition().enabled_tracks = d.dispositionTrackEnablement;
  ...
  if (sandboxOptions.compliance_and_risk_gates) {
    ...
    if (c.bankruptcyStayGateLegalBlock !== undefined) comp.bankruptcy_stay_gate_enabled = c.bankruptcyStayGateLegalBlock;
    if (c.fha90DayResaleRuleGate !== undefined) comp.fha_90_day_gate_enabled = c.fha90DayResaleRuleGate;
    if (c.firptaWithholdingGate !== undefined) comp.firpta_gate_enabled = c.firptaWithholdingGate;
    if (c.flood50RuleGate !== undefined) comp.flood_50_gate_enabled = c.flood50RuleGate;
    ...
    if (c.scraVerificationGate !== undefined) comp.scra_gate_enabled = c.scraVerificationGate;
    if (c.stateProgramGateFhaVaOverlays !== undefined) comp.fha_va_overlays_gate_enabled = c.stateProgramGateFhaVaOverlays;
  ...
  if (sandboxOptions.timeline) {
    ...
    if (sandboxOptions.timeline.daysToMoneySelectionMethod) {
      policy.dtm.selection_method = sandboxOptions.timeline.daysToMoneySelectionMethod as any;
    }
  ...
  if (sandboxOptions.workflow_and_guardrails) {
    policy.workflow_policy = {
      analyst_review_borderline_threshold:
        sandboxOptions.workflow_and_guardrails.analystReviewTriggerBorderlineBandThreshold ?? null,
      cash_presentation_min_spread_over_payoff:
  ...
  if (sandboxOptions.ux_policy) {
    policy.ux_policy = {
      ...
      buyer_costs_dual_scenario_when_unknown:
