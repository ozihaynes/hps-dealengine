...
export function useFieldModeData(): UseFieldModeDataResult {
  const { dbDeal, lastAnalyzeResult, lastRunId, isHydratingActiveDeal } = useDealSession();
  ...
  return useMemo(() => {
    ...
    const payload = dbDeal.payload as Record<string, unknown> | null;
    const market = payload?.market as Record<string, unknown> | undefined;
    const arv = (market?.arv as number) ?? null;
  ...
  }, [dbDeal, lastAnalyzeResult, lastRunId, isHydratingActiveDeal]);
}
