...
export default function PolicyOverridesSettingsPage() {
  ...
  useEffect(() => {
    setLoading(true);
    setError(null);
    // Fetch all for the org by selecting by deal_id via RLS; using empty run filter (deal required by RLS)
    supabase
  ...
  const handleDecision = async (id: string, decision: "approved" | "rejected") => {
    ...
    try {
      ...
      setOverrides((prev) =>
        prev.map((o) => (o.id === id ? updated : o)).filter((o) => o.status === "pending"),
      );
    } catch (err: unknown) {
  ...
  return (
    <div className="space-y-6">
      ...
      <GlassCard className="p-4 border border-white/5 bg-surface-elevated/60">
        {error && (
        ...
        {loading ? (
          ...
          <div className="overflow-auto">
            <table className="min-w-full text-sm">
              ...
              <tbody>
                {overrides.map((o) => (
                  <tr key={o.id} className="border-b border-border-subtle/40 last:border-b-0">
                    ...
                    <td className="px-2 py-1 align-top">
                      {canApprove ? (
                        <div className="flex gap-2">
                          <Button size="sm" variant="primary" onClick={() => void handleDecision(o.id, "approved")}>
                            Approve
                          </Button>
                          <Button size="sm" variant="danger" onClick={() => void handleDecision(o.id, "rejected")}>
                            Reject
                          </Button>
                        </div>
                      ) : (
