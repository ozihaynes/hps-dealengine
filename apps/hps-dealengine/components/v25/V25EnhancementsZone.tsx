/**
 * V25EnhancementsZone — Slice 13.3 + 14 + Score Analysis
 *
 * Enhancement zone that renders V2.5 components ABOVE the Command Center.
 * Complete layout with 11 rows:
 *
 * ROW 0: Decision Hero (verdict reveal animation)
 * ROW 1: Score Analysis Bar (Closeability, Urgency, Buyer Demand)
 * ROW 2: Confidence Indicators Bar (Comp Quality, Evidence, Market, ARV)
 * ROW 3: Status Bar (Risk Gates + Evidence Health)
 * ROW 4: Verdict Card (detailed view)
 * ROW 5: Price Geometry Bar (visual ZOPA)
 * ROW 6: Net Clearance Panel (exit strategies)
 ...
 * - Miller's Law: Manageable number of items per row
 * - Doherty Threshold: Animations under 400ms feel instant
 *
 * @module components/v25/V25EnhancementsZone
 * @version 2.5.0 (Score Analysis + Decision Rationale)
 */
...
import { DEMO_ENGINE_OUTPUTS, DEMO_RATIONALE } from "@/lib/constants/demoData";
...
// Score Analysis Bar + Decision Rationale
import { ScoreAnalysisBar, DecisionRationale, type ScoreAnalysisData } from "@/components/dashboard/scores";
...
interface EngineOutputs {
  ...
  score_analysis?: ScoreAnalysisData | null;
}
...
export function V25EnhancementsZone({ className }: V25EnhancementsZoneProps) {
  ...
  if (isHydratingActiveDeal) {
    return (
      <div className={cn("space-y-4 animate-pulse", className)}>
        <div className="h-32 bg-[var(--card-bg)] rounded-xl border border-white/10 backdrop-blur-xl" />
        <div className="h-20 bg-[var(--card-bg)] rounded-xl border border-white/10 backdrop-blur-xl" />
        <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
          <div className="h-24 bg-[var(--card-bg)] rounded-xl border border-white/10 backdrop-blur-xl" />
          <div className="h-24 bg-[var(--card-bg)] rounded-xl border border-white/10 backdrop-blur-xl" />
          <div className="h-24 bg-[var(--card-bg)] rounded-xl border border-white/10 backdrop-blur-xl" />
          <div className="h-24 bg-[var(--card-bg)] rounded-xl border border-white/10 backdrop-blur-xl" />
        </div>
  ...
  // Score analysis with factor breakdowns
  const scoreAnalysis = outputs.score_analysis ?? null;

  // Rationale points for Decision Rationale component
  const rationalePoints = isDemo ? DEMO_RATIONALE : [];
  ...
  return (
    ...
    >
      ...
      {/* ═══════════════════════════════════════════════════════════════════
          ROW 1: Score Analysis Bar
          3 expandable cards: Closeability, Urgency, Buyer Demand
          ═══════════════════════════════════════════════════════════════════ */}
      <motion.div variants={itemVariants}>
        <ScoreAnalysisBar
          data={scoreAnalysis}
      ...
      {/* ═══════════════════════════════════════════════════════════════════
          ROW 2: Confidence Indicators Bar (Slice 15)
      ...
      {/* ═══════════════════════════════════════════════════════════════════
          ROW 3: Status Bar (Slice 17) — Risk Gates + Evidence Health
      ...
      {/* ═══════════════════════════════════════════════════════════════════
          ROW 4: Verdict Card (Decision Hero — Detailed View)
          ═══════════════════════════════════════════════════════════════════ */}
      <motion.div variants={itemVariants}>
        <VerdictCard
          verdict={verdict}
          ...
          showConfidence={true}
          className="bg-[var(--card-bg-active)]"
        />
      </motion.div>
      ...
      {/* ═══════════════════════════════════════════════════════════════════
          ROW 10: Decision Rationale
          Verdict reasoning with supporting factors
          ═══════════════════════════════════════════════════════════════════ */}
      <motion.div variants={itemVariants}>
        <DecisionRationale
          verdict={verdict}
