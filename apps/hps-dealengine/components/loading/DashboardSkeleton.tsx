  ...
  return (
    ...
    >
      {Array.from({ length: count }).map((_, index) => (
        <div
          key={index}
          className="rounded-xl border border-white/10 bg-[var(--card-bg)] backdrop-blur-xl p-4"
        >
  ...
  return (
    ...
    >
      {Array.from({ length: count }).map((_, index) => (
        <div
          key={index}
          className="rounded-xl border border-white/10 bg-[var(--card-bg)] backdrop-blur-xl p-5"
        >
          {/* Section header */}
          ...
          <div className="space-y-3">
            {Array.from({ length: 3 }).map((_, itemIndex) => (
              <div
                key={itemIndex}
                className="flex items-center gap-3 p-3 rounded-lg bg-[var(--card-bg-hero)]"
              >
    ...
    >
      ...
      <div className={cn(showSidebar && 'flex gap-6')}>
        ...
        {showSidebar && (
          ...
          >
            <div className="rounded-xl border border-white/10 bg-[var(--card-bg)] backdrop-blur-xl p-4 space-y-3">
              <ShimmerText width="80%" height={14} />
        ...
        <motion.div variants={sectionVariants} className="flex-1">
          {contentVariant === 'cards' && (
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
              {Array.from({ length: 6 }).map((_, index) => (
                <div
                  key={index}
                  className="rounded-xl border border-white/10 bg-[var(--card-bg)] backdrop-blur-xl p-4 space-y-3"
                >
          ...
          {contentVariant === 'list' && (
            <div className="space-y-3">
              {Array.from({ length: 5 }).map((_, index) => (
                <div
                  key={index}
                  className="flex items-center gap-4 p-4 rounded-xl border border-white/10 bg-[var(--card-bg)] backdrop-blur-xl"
                >
          ...
          {contentVariant === 'table' && (
            <div className="rounded-xl border border-white/10 bg-[var(--card-bg)] backdrop-blur-xl overflow-hidden">
              {/* Header */}
