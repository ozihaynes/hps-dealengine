...
const ScenarioModeler: React.FC<ScenarioModelerProps> = ({ deal, setDealValue, sandbox, calc }) => {
  ...
  const scenarioResult = useMemo(() => {
    ...
    // FIX: Pass the sandbox settings to the runEngine function.
    return HPSEngine.runEngine({ deal: scenarioDealData }, sandbox);
  }, [deal, scenarioDays, scenarioRepairs, scenarioConcessions, sandbox]);

  const scenarioCalc = scenarioResult?.calculations ?? null;
  // FIX: Use the `calc` prop for the original calculation instead of calling the hook incorrectly.
  const originalCalc: any = (calc as any)?.calculations ?? (calc as any) ?? null;
  // Guard: wait for both calcs (auto-inserted)
  if (!originalCalc || !scenarioResult || !scenarioResult.calculations) {
    return (
      <div className="card">
        <h3>Scenario Modeler</h3>
        <div className="muted">Results will appear after analysis runs.</div>
      </div>
    );
  }
          ...
          <tbody>
            <tr>
              ...
              <td className="py-2 text-right font-mono">
                {fmt$(originalCalc?.instantCashOffer ?? 0, 0)}
              </td>
              <td className="py-2 text-right font-mono font-bold text-lg text-yellow-300">
                {fmt$(scenarioCalc?.instantCashOffer ?? 0, 0)}
              </td>
              <td className="py-2 text-right font-mono">
                {renderDelta(
                  originalCalc?.instantCashOffer ?? 0,
                  scenarioCalc?.instantCashOffer ?? 0
                )}
              </td>
            </tr>
            <tr className="border-t border-text-primary/10">
              ...
              <td className="py-2 text-right font-mono">
                {fmt$(originalCalc?.netToSeller ?? 0, 0)}
              </td>
              <td className="py-2 text-right font-mono font-bold text-lg text-yellow-300">
                {fmt$(scenarioCalc?.netToSeller ?? 0, 0)}
              </td>
              <td className="py-2 text-right font-mono">
                {renderDelta(originalCalc?.netToSeller ?? 0, scenarioCalc?.netToSeller ?? 0)}
              </td>
            </tr>
            <tr className="border-t border-text-primary/10">
              ...
              <td className="py-2 text-right font-mono text-text-secondary/70">
                {(originalCalc?.urgencyDays ?? 0) > 0 ? `${originalCalc?.urgencyDays}d` : '—'}
              </td>
              <td className="py-2 text-right font-mono text-text-secondary/70">
                {(scenarioCalc?.urgencyDays ?? 0) > 0 ? `${scenarioCalc?.urgencyDays}d` : '—'}
              </td>
              <td className="py-2 text-right font-mono">
                {renderDelta(
