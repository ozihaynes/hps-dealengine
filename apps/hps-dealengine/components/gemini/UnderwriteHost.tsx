 ...
 * We keep this lightweight — the real math stays server-side in /api/analyze.
 */
const defaultDeal: any = {
  ...
  debt: {
    ...
    senior_per_diem: 0,
    good_thru_date: new Date().toISOString(),
    juniors: [],
    hoa_arrears: 0,
    muni_fines: 0,
...
export default function UnderwriteHost() {
  ...
  async function pingAnalyze() {
    try {
      ...
      console.log('analyze:', json);
      alert(json?.ok ? 'Analyze OK — check console' : `Analyze error: ${json?.error ?? 'unknown'}`);
    } catch (e) {
      console.error('pingAnalyze error', e);
      const msg = e instanceof Error ? e.message : typeof e === 'string' ? e : JSON.stringify(e);
      alert(`Analyze threw: ${msg}`);
    } finally {
      setPinging(false);
    }
  }
  ...
  return (
    <>
      <UnderwriteTab deal={deal} calc={calc} setDealValue={setDealValue} sandbox={sandbox} />
