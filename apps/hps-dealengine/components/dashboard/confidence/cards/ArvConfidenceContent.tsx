  ...
  return (
    <div className="space-y-3" data-testid="arv-confidence-content">
      ...
      <div className="space-y-2">
        ...
        {/* Visual bar - NO GRADIENT, solid fill based on confidence */}
        <div className="relative h-10 bg-slate-800 rounded-lg overflow-hidden">
          {/* Solid range fill based on confidence */}
          <div
            className={cn(
          ...
          {/* Mid marker */}
          <div className={cn(
            "absolute top-0 bottom-0 w-0.5 left-1/2 transform -translate-x-1/2",
            confidence === "high"
              ? "bg-emerald-500"
              : confidence === "medium"
                ? "bg-amber-500"
      ...
      {spread !== null && spreadPct !== null && (
        <div className="grid grid-cols-2 gap-2 pt-2 border-t border-white/10">
          <div className="text-sm">
            <span className="text-slate-500 block text-xs">Range Spread</span>
            <span className="text-slate-300 font-medium">
              {formatCurrency(spread)}
            </span>
          </div>
          <div className="text-sm">
            <span className="text-slate-500 block text-xs">Spread %</span>
            <span className={cn(
              "font-medium",
