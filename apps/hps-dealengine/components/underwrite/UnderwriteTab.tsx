...
const UnderwriteTab: React.FC<UnderwriteTabProps> = ({ deal, calc, setDealValue, sandbox }) => {
  // ------- SAFE ALIASES (no crashes when nested objects are missing) -------
  const market   = (deal?.market   ?? {}) as any;
  const debt     = (deal?.debt     ?? {}) as any;
  const property = (deal?.property ?? {}) as any;
  const status   = (deal?.status   ?? {}) as any;
  const title    = (deal?.title    ?? {}) as any;
  const policy   = (deal?.policy   ?? {}) as any;
  const timeline = (deal?.timeline ?? {}) as any;
  const legal    = (deal?.legal    ?? {}) as any;

  // ------- Small helpers -------
  const addJuniorLien = () => {
    const newLien = { id: Date.now(), type: 'Judgment', balance: '', per_diem: '', good_thru: '' };
    setDealValue('debt.juniors', [...(debt.juniors ?? []), newLien]);
  };

  const updateJuniorLien = (index: number, field: string, value: any) => {
    const updated = [...(debt.juniors ?? [])];
    if (!updated[index]) updated[index] = {};
    updated[index][field] = value;
    setDealValue('debt.juniors', updated);
  };

  const removeJuniorLien = (index: number) => {
    setDealValue('debt.juniors', (debt.juniors ?? []).filter((_: any, i: number) => i !== index));
  };
          ...
          <NestedTabsContent value="market">
            <div className="grid grid-cols-2 md:grid-cols-4 gap-3 mt-3">
              ...
              <InputField
                ...
                type="number"
                suffix="%"
                value={(market['price-to-list-pct'] ?? 0) * 100}
              ...
              <InputField
                ...
                type="number"
                suffix="%"
                value={(market.local_discount_20th_pct ?? 0) * 100}
          ...
          <NestedTabsContent value="property">
            <div className="grid grid-cols-1 md:grid-cols-3 gap-3 mt-3">
              <div className="space-y-3">
                <SelectField
                  label="Occupancy"
                  value={property.occupancy ?? 'owner'}
                ...
                <SelectField
                  ...
                  value={status.insurability ?? 'bindable'}
              ...
              <div className="info-card space-y-3 p-4">
                <ToggleSwitch
                  ...
                  onChange={() =>
                    setDealValue('confidence.no_access_flag', !Boolean(deal?.confidence?.no_access_flag))
                  }
                />
                <ToggleSwitch
                  ...
                  onChange={() =>
                    setDealValue('status.structural_or_permit_risk_flag', !Boolean(status.structural_or_permit_risk_flag))
                  }
                />
                <ToggleSwitch
                  label="Old Roof (>20 years)"
                  checked={!!property.old_roof_flag}
                  onChange={() => setDealValue('property.old_roof_flag', !Boolean(property.old_roof_flag))}
                />
              </div>
              ...
              <div className="info-card space-y-3 p-4">
                <ToggleSwitch
                  ...
                  onChange={() =>
                    setDealValue('status.major_system_failure_flag', !Boolean(status.major_system_failure_flag))
                  }
                />
                <ToggleSwitch
                  ...
                  onChange={() =>
                    setDealValue(
                      ...
                      !Boolean(deal?.confidence?.reinstatement_proof_flag)
                    )
                  }
                />
                <ToggleSwitch
                  ...
                  onChange={() => setDealValue('property.is_homestead', !Boolean(property.is_homestead))}
                />
              </div>
            </div>
          </NestedTabsContent>
          ...
          <NestedTabsContent value="debt">
              ...
              <FieldGroup title="Senior Lien Payoff">
                <div className="grid grid-cols-2 gap-3 items-end">
                  ...
                  <ToggleSwitch
                    ...
                    onChange={() => setDealValue('debt.payoff_is_confirmed', !Boolean(debt.payoff_is_confirmed))}
                  />
                  <InputField
              ...
              <FieldGroup title="Other Encumbrances & Title">
                <div className="grid grid-cols-2 gap-3 items-end">
                  ...
                  <InputField
                    ...
                    min="0"
                    max="3"
                    value={((title.risk_pct ?? 0) as number) * 100}
            ...
            <div className="mt-4 pt-4">
              <div className="flex justify-between items-center mb-2">
                ...
                <Button size="sm" variant="neutral" onClick={addJuniorLien}>+ Add Lien</Button>
              </div>
          ...
          <NestedTabsContent value="policy">
            <div className="grid grid-cols-2 md:grid-cols-4 gap-3 mt-3">
              ...
              <InputField
                ...
                type="number"
                suffix="%"
                value={policy.list_commission_pct != null ? (policy.list_commission_pct as number) * 100 : ''}
                onChange={(e) =>
                  setDealValue(
              ...
              <InputField
                ...
                type="number"
                suffix="%"
                value={((deal?.costs?.sell_close_pct ?? 0) as number) * 100}
                onChange={(e) => setDealValue('costs.sell_close_pct', parseFloat(e.target.value) / 100)}
                min="0"
                max="30"
                step="0.1"
              ...
              <InputField
                ...
                type="number"
                suffix="%"
                value={((deal?.costs?.concessions_pct ?? 0) as number) * 100}
                onChange={(e) => setDealValue('costs.concessions_pct', parseFloat(e.target.value) / 100)}
                min="0"
                max="30"
                step="0.1"
              ...
              <InputField
                ...
                type="number"
                suffix="%"
                value={((policy.safety_on_aiv_pct ?? 0) as number) * 100}
                onChange={(e) => setDealValue('policy.safety_on_aiv_pct', parseFloat(e.target.value) / 100)}
                min="0"
                max="10"
                step="0.1"
              ...
              <ToggleSwitch
                ...
                onChange={() => setDealValue('policy.costs_are_annual', !Boolean(policy.costs_are_annual))}
              />
            </div>
          </NestedTabsContent>
            ...
            <div className="grid grid-cols-2 md:grid-cols-4 gap-3 mt-3 items-end">
              <div className="pt-4 md:col-span-2">
                <ToggleSwitch
                  ...
                  onChange={() => setDealValue('property.is_foreclosure_sale', !Boolean(property.is_foreclosure_sale))}
                />
              </div>
              <div className="pt-4 md:col-span-2">
                <ToggleSwitch
                  ...
                  onChange={() =>
                    setDealValue('property.is_redemption_period_sale', !Boolean(property.is_redemption_period_sale))
                  }
                />
              </div>
              <InputField
              ...
              <InputField
                ...
                onChange={(e) =>
                  setDealValue('policy.manual_days_to_money', e.target.value === '' ? null : e.target.value)
                }
              />
            </div>
          </NestedTabsContent>
