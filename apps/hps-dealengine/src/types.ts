...
export type EngineDeal = {
  ...
  costs?: {
    repairs_base?: number;
    contingency_pct?: number; // 0.15 => 15%
    essentials_moveout_cash?: number;
    monthly?: {
      taxes?: number; // annual (we’ll /12), per SOT convention
      insurance?: number; // annual (we’ll /12)
      hoa?: number; // monthly
  ...
  debt?: {
    ...
    juniors?: Array<{ label?: string; amount?: number }>;
  };
  timeline?: {
    days_to_ready_list?: number;
    days_to_sale_manual?: number; // if provided, we’ll consider earliest-of vs DOM+X
  };
  status?: {
...
export type HeadlinesOut = {
  ...
  net_to_seller: Money | null; // for now: RF operational; refine with closing math later
  notes?: string[];
};
