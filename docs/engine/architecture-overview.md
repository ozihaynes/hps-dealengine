...
| `supabase/functions/v1-analyze` | Underwriting entrypoint; resolves org + active policy version under RLS, builds policy snapshot, calls `packages/engine`, returns outputs + trace + policy linkage | Deterministic; no persistence. |
...
**Data flow (one pass)**: UI (/underwrite) → build AnalyzeInput + policy opts → `v1-analyze` resolves policy snapshot + linkage → `packages/engine` → outputs/trace → user reviews → `v1-runs-save` persists → `/overview` & `/trace` read run/trace → AI (via `v1-ai-bridge`) explains using same run.
...
3. `v1-analyze` resolves org + active policy version (RLS), invokes `packages/engine`:
   - Returns `AnalyzeOutput` + `trace` + policy snapshot linkage (`policyVersionId`, `policyHash`, `policySnapshot`).
4. User can preview; to persist, client calls `v1-runs-save` with outputs/trace/policy snapshot/`policyVersionId`/hashes/deal_id/posture.
...
- `runs` store: org_id, deal_id, posture, policy_version_id, input/output/trace, policy_snapshot, hashes (input/policy/output), workflow_state, created_by/at.
...
- `v1-analyze`: Underwriting; input = deal + policy options; output = AnalyzeOutput + trace + policy linkage; no persistence.
