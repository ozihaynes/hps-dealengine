 ...
 *
 * KEY BEHAVIOR: Always returns valid snapshot data.
 * - If no analysis exists, returns default placeholder values
 * - Dashboard is always visible with structure intact
 ...
 */
const DEFAULT_SNAPSHOT: SnapshotData = {
...
export function useOverviewData(): OverviewData {
  ...
  const { snapshot, hasRealData } = useMemo<{ snapshot: SnapshotData; hasRealData: boolean }>(() => {
  ...
  const signals = useMemo<Signal[]>(() => {
    ...
    const analysisSignals = generateSignalsFromAnalysis(dealId, outputs, trace);
