...
### 2026-01-16 - Slice 6 (Ensemble + Uncertainty core)

- DB: Migration `20260108113000_valuation_ensemble_uncertainty_tokens.sql` seeds ACTIVE policies (missing keys only) with valuation.ensemble defaults (enabled=false, version ensemble_v1, weights comps 0.7 / avm 0.3, max_avm_weight 0.5, min_comps_for_avm_blend 3) and valuation.uncertainty defaults (enabled=false, version uncertainty_v1, method weighted_quantiles_v1, p_low 0.10, p_high 0.90, min_comps 3, floor_pct 0.05). Applied via `SUPABASE_PROJECT_REF=zjkihnihhqmnhpxkecpy supabase db push`.
- Engine/Edge: Added deterministic helpers for ensemble (blend comps/AVM with policy weights + optional ceiling cap) and uncertainty (weighted quantiles with optional AVM blend + floor width). `v1-valuation-run` wires optional ensemble/uncertainty outputs, gates input_hash on enabled configs, and leaves selection/outlier behavior unchanged. Deployed `supabase functions deploy v1-valuation-run --project-ref zjkihnihhqmnhpxkecpy`.
- Contracts: suggested_arv_basis allows `ensemble_v1`; output now includes optional ensemble_* (version/weights/estimates/cap) and uncertainty_* (version/method/range/range_pct) fields.
- Proof: `scripts/valuation/prove-ensemble-uncertainty.ps1` (caller JWT, patch active policies by id, restore in finally) shows baseline vs enabled hash change and determinism: baseline input_hash `f36a52eb7cbd821f9a3e954d0c54308895b556e48385c904d2cd62c594b5edf9` → ensemble input_hash `47bca9f7aa0725705b00bcea3aac9106ed2e8dfd7eeb88606897014fd9ea1c06`; Run2/Run3 output_hash equal True; run_hash equal True; policies restored. Coverage smoke PASS (HPS_SMOKE_EMAIL/HPS_SMOKE_PASSWORD env).
