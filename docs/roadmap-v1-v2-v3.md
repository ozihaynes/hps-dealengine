...
- ✅ Slice 6 (ensemble + uncertainty): active policies seeded with ensemble/uncertainty defaults (OFF) via `20260108113000_valuation_ensemble_uncertainty_tokens.sql`; `v1-valuation-run` blends comps+AVM with policy weights/cap when enabled, emits ensemble_* + uncertainty_* outputs, gates input_hash on enabled configs, keeps selection/outlier logic unchanged. Proof `scripts/valuation/prove-ensemble-uncertainty.ps1`: baseline input_hash `f36a52eb7cbd821f9a3e954d0c54308895b556e48385c904d2cd62c594b5edf9` -> ensemble input_hash `47bca9f7aa0725705b00bcea3aac9106ed2e8dfd7eeb88606897014fd9ea1c06`; Run2/Run3 hashes equal; policies restored. Deploys: `supabase db push` (project zjkihnihhqmnhpxkecpy) + `supabase functions deploy v1-valuation-run --project-ref zjkihnihhqmnhpxkecpy`.
- ?? In progress
