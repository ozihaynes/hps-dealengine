 ...
 * - EC-2.8: Remove owner from org - 403 "Cannot remove owner"
 *
...
serve(async (req: Request): Promise<Response> => {
  ...
  try {
    ...
    // EC-2.8: Cannot remove owner (check if target is org creator/owner)
    // Since we don't have an explicit "owner" role, check if they're the last manager
    // OR check against org.created_by if that exists
    ...
    // For now, we'll allow removal of any non-VP members
    // VP role is typically the highest and should be protected
    ...
    // Check if this would leave the org with no managers
