...
The settings page at `apps/hps-dealengine/app/(app)/settings/user/page.tsx` had:

1. **Fake/stub data** - `LocalTeamMember` type with hardcoded members
2. **"(UI-only)" labels** - Indicating functionality not connected
3. **Non-functional invite form** - Button did nothing
4. **Non-functional remove buttons** - Just updated local state
