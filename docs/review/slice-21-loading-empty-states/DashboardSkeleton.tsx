...
const HeroSkeleton = memo(function HeroSkeleton(): React.ReactElement {
  return (
    <motion.div
      ...
      className={cn(
        'rounded-xl border border-zinc-700/50 bg-zinc-800/50',
        'p-6'
      )}
        ...
        <motion.div variants={sectionVariants} className="flex-1">
          {contentVariant === 'cards' && (
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
              {Array.from({ length: 6 }).map((_, index) => (
                <div
                  key={index}
                  className="rounded-lg border border-zinc-700/50 bg-zinc-800/50 p-4 space-y-3"
                >
          ...
          {contentVariant === 'list' && (
            <div className="space-y-3">
              {Array.from({ length: 5 }).map((_, index) => (
                <div
                  key={index}
                  className="flex items-center gap-4 p-4 rounded-lg border border-zinc-700/50 bg-zinc-800/50"
                >
          ...
          {contentVariant === 'table' && (
            <div className="rounded-lg border border-zinc-700/50 bg-zinc-800/50 overflow-hidden">
              {/* Header */}
              <div className="flex items-center gap-4 p-4 border-b border-zinc-700/50 bg-zinc-800">
                {Array.from({ length: 5 }).map((_, index) => (
                  <ShimmerText
                    key={index}
              ...
              {Array.from({ length: 5 }).map((_, rowIndex) => (
                <div
                  key={rowIndex}
                  className="flex items-center gap-4 p-4 border-b border-zinc-700/30 last:border-b-0"
                >
