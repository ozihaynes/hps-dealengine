...
function RiskGatesDetailContent({ gates }: { gates: GateData[] }): JSX.Element {
  const STATUS_COLORS: Record<GateStatus, string> = {
    ...
    unknown: "border-zinc-500/30 bg-zinc-500/10",
  };
        ...
        {gates.map((gate) => (
          ...
          >
            <div className="flex items-center justify-between">
              <span className="font-medium text-zinc-100">{gate.label}</span>
              <GateIcon
                gateId={gate.id}
                label={gate.label}
                status={gate.status}
    ...
    >
      ...
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-2">
          <span className="text-sm font-medium text-zinc-300">Risk Gates</span>
          {isDemoMode && (
        ...
        <span className={cn(
          ...
          overallStatus === "unknown" && "text-zinc-400"
        )}>
