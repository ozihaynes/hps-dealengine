...
function AppTabNav({ onOpenOffer }: { onOpenOffer?: () => void }) {
  ...
  return (
    <nav className="flex w-full flex-wrap items-center justify-between gap-2 rounded-xl border border-border-subtle bg-surface-raised px-2 py-1 text-sm text-text-primary backdrop-blur">
      <div className="flex flex-wrap items-center gap-2">
        ...
        {dbDeal?.id ? (
          ...
          <Link
            ...
            className="tab-trigger"
          >
            <Icon d={Icons.check} size={16} className="text-text-secondary" />
            <span>Offer</span>
          </Link>
        )}
      </div>
            ...
            <main className="flex-1">
              <div className="mx-auto flex max-w-6xl lg:max-w-[96rem] xl:max-w-[104rem] 2xl:max-w-[112rem] flex-col gap-4 px-6 lg:px-8 xl:px-10 py-6">
                ...
                {/* ErrorBoundary scoped to route children only.
                    ...
                    excluded so navigation remains functional during page errors.
                    ToastProvider is outside so error notifications still work. */}
                <ErrorBoundary>
                  <Suspense
