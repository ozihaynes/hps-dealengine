...
export interface ShimmerOverlayProps {
  ...
  /** Base color (default: zinc-700) */
  baseColor?: string;
  /** Highlight color (default: zinc-600) */
...
export function ShimmerOverlay({
  ...
  highlightColor = '#52525b', // zinc-600
  duration = 1.5,
  className = '',
...
export function SkeletonCard({
  ...
  return (
    <div
      className={`p-4 rounded-lg border border-zinc-800 bg-zinc-900/50 ${className}`}
    >
      {/* Header */}
...
export function SkeletonTable({
  ...
  return (
    <div className={className}>
      {/* Header */}
      <div
        className="flex gap-4 pb-3 border-b border-zinc-800 mb-2"
        style={{ height: 32 }}
      >
      ...
      {Array.from({ length: rows }).map((_, rowIndex) => (
        <div
          key={`row-${rowIndex}`}
          className="flex items-center gap-4 border-b border-zinc-800/50"
