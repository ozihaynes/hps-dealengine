...
const BAND_CONFIG = {
  ...
  balanced: { label: "Balanced", color: "bg-blue-500", text: "text-blue-400", border: "border-blue-500/30" },
  cool: { label: "Cool Market", color: "bg-cyan-500", text: "text-cyan-400", border: "border-cyan-500/30" },
  cold: { label: "Cold Market", color: "bg-zinc-500", text: "text-zinc-400", border: "border-zinc-500/30" },
} as const;
  ...
  return (
    <div className="space-y-3" data-testid="market-velocity-content">
      ...
      {(cashBuyer !== null || data.active_listings !== null || data.yoy_price_change_pct !== null) && (
        <div className="pt-2 border-t border-zinc-700/50 grid grid-cols-3 gap-2 text-xs text-center">
          {cashBuyer !== null && (
            <div>
              <div className="text-zinc-300 font-medium">{cashBuyer.toFixed(0)}%</div>
              <div className="text-zinc-500">Cash Buyers</div>
            </div>
          )}
          {data.active_listings !== null && (
            <div>
              <div className="text-zinc-300 font-medium">{data.active_listings}</div>
          ...
          {data.yoy_price_change_pct != null && (
            <div>
              ...
              <div className="text-zinc-500">YoY</div>
            </div>
          )}
        </div>
      )}
      ...
      {/* Data source */}
      {data.data_source && (
        <div className="pt-2 border-t border-zinc-700/50 flex items-center justify-between text-xs">
          <span className="text-zinc-500">Source: {data.data_source}</span>
          {data.data_age_days !== null && (
            <span className="text-zinc-500">{data.data_age_days}d ago</span>
          )}
        </div>
      )}
    </div>
