    ...
    >
      {Array.from({ length: count }).map((_, index) => (
        <motion.div
          key={index}
          variants={itemVariants}
          className="flex items-center gap-4 p-4 rounded-xl border border-white/10 bg-slate-800/50 backdrop-blur-xl"
        >
          {/* Image placeholder */}
          <div className="w-16 h-16 rounded-lg bg-slate-700/50 shrink-0" />
