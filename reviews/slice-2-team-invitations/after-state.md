...
The settings page now has a fully functional "Team access" card:

1. **Real API data** - Uses `TeamMember` and `InvitationDisplay` types from contracts
2. **No "(UI-only)" label** - All functionality is connected
3. **Invite form works** - Sends real invitations via Resend
4. **Remove buttons work** - Actually removes members via API
